...
public class SyslogMessageMapperTest extends DatabaseBaseTest {
	...
	// mapped well
	List<Message> dbMessages = mapper.getMessages();
	...
	SyslogSessionMapper s_mapper = null;
	try {
	    s_mapper = new SyslogSessionMapper(db);
	} catch (Exception e) {
	    fail(StackTraceUtil.toString(e));
	}

	assertNotNull(s_mapper);
	...
	for (int i = 0; i < iterationsCount; ++i) {
		...
		Message msg = new Message(RandomUtils
			.getRandomString(msgTextLen), RandomUtils
			.getRandomString(msgAttributeLen), RandomUtils
			.getRandomString(msgAttributeLen), RandomUtils
			.getRandomString(msgAttributeLen), session);
		assertTrue(mapper.save(msg));
		// db.commit();
	    }
	    db.commit();
