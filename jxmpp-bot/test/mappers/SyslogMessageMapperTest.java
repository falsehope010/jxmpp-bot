...
public class SyslogMessageMapperTest extends DatabaseBaseTest {
	...
	/**
	 * Gets all syslog messages from database, finds message with latest date
	 * and returns it's timestamp property
	 * 
	 * @param db
	 *            Valid connected database
	 * @return Timestamp of latest syslog message in database if succeded, null
	 *         otherwise
	 */
	private Date getLatestMessageDate(Database db) {
		Date result = null;
		...
		try {
			...
			Date max = records.get(0).getDate("timestamp");
		} catch (Exception e) {
			fail(StackTraceUtil.toString(e));
		}
